...
A3L_TFEnabled = true;
A3L_onTsServer = "[GER] Entzugsklinik Gaming - Arma 3 ModServer" == (call TFAR_fnc_getTeamSpeakServerName);
	...
	while {true} do {
		...
		if (!(_isadmin)) then {
			...
			if ((!(_TFenabled)) && (A3L_TFEnabled)) then {
				titleText ["Bitte aktiviere das Task Force Radio TS3 Plugin! || TS3 -> Settings -> Plugins", "BLACK"];
				A3L_TFEnabled = false;
			};
			_onTsServer = "[GER] Entzugsklinik Gaming - Arma 3 ModServer" == (call TFAR_fnc_getTeamSpeakServerName);
			if (!(_onTsServer)) then {
				titleText ["Bitte joine dem Entzugsklinik Teamspeak!", "BLACK"];
				A3L_onTsServer = false;
			} else {
				if (!(A3L_onTsServer)) then {
					titleText ["Teamspeak Abfrage war Erfolgreich!","BLACK IN"];
					A3L_onTsServer = true;
				};
			};
			_onChannel = "TaskForceRadio" == (call TFAR_fnc_getTeamSpeakChannelName);
			if (!(_onChannel)) then {
				titleText ["Bitte aktiviere das Task Force Radio TS3 Plugin! || Settings -> Plugins -> Reload All", "BLACK"];
				A3L_onChannel = false;
			} else {
			...
			if ((_TFenabled) && (!(A3L_TFEnabled))) then {
				titleText ["TFAR Plugin Wurde Erfolgreich Aktiviert!","BLACK IN"];
				A3L_TFEnabled = true;
			};
		};
		sleep 2;	
...
[] execVM "core\functions\fn_fps.sqf";//fps hud
...
[] execVM "core\fn_addKey.sqf";///SCHLÜSSEL RÜCKGABE
[] execVM "core\TJMS\MasterCONF.sqf"; //Initiate TJMS\MasterCONF
[] spawn life_fnc_autoSave; // hm.. SAVE!
[] spawn life_fnc_fatigueReset; //reset it bitch!
